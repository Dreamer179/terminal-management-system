...
class CameraAdmin(admin.ModelAdmin):
    list_display = ('camera_id', 'camera_name', 'use_for_ocr', 'stream_url', 'status', 'action',) #'stream'
    ...
    def action(self, obj):
        log.debug(f'CameraAdmin ACTION')
        worker_config = None
        if obj.use_for_ocr:
    ...
    def config_worker(cls, request, camera_id):
        try:
            log.debug(f'CameraAdmin CONFIG WORKER ...')
    ...
    def start_worker(cls, request, camera_id):
        try:
            log.debug(f'CameraAdmin START WORKER...')
            from container.models import Camera, WorkerConfig
            ...
            log.debug(f'Start worker for camera {camera_id} fail, {traceback.format_exc()}')
            messages.add_message(
                request,
                messages.ERROR,
    ...
    def stop_worker(cls, request, camera_id):
        try:
            log.debug(f'CameraAdmin STOP WORKER ...')
            from container.models import Camera, WorkerConfig
            camera = Camera.objects.get(camera_id=camera_id)
            if not camera.use_for_ocr:
                raise ValueError(f'camera is not used for ocr')
    ...
    def live_stream(cls, request, camera_id):
        log.debug(f'CameraAdmin LIVE STREAM ...')
